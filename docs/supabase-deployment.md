...
anon key 会进入浏览器构建产物，属于公开客户端凭据；真正需要保护的是 Function secret 和 service role key。
...
立即在 Supabase 中轮换 `SCORE_TOKEN_SECRET` 或 service role key，并重新部署 Function。已发布到浏览器的 anon key 不属于需要轮换的服务端秘密。
